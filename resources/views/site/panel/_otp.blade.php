...
h1{font-size:19px;text-align:center;margin:0 0 6px;color:#1e3a8a;}
.subtitle{text-align:center;font-size:13.5px;color:#64748b;margin-bottom:20px;}
.subtitle b{color:#1e3a8a;direction:ltr;display:inline-block;}
...
.field input.code-input{text-align:center;letter-spacing:6px;font-size:20px;font-weight:bold;padding:12px 16px;direction:ltr;}
.ic{position:absolute;right:12px;top:50%;transform:translateY(-50%);width:20px;height:20px;pointer-events:none;background:transparent;mix-blend-mode:multiply;}
...
.resend-form{margin-top:2px;}
.resend-btn{display:block;width:100%;background:none;border:0;color:#2563eb;font-weight:bold;font-size:13.5px;cursor:pointer;padding:6px;font-family:inherit;}
...
@if ($mode === 'verify')
    ...
    <div class="subtitle">کد ۶ رقمی ارسال‌شده به <b>{{ $mobile }}</b> را وارد کنید</div>
@elseif ($mode === 'forgot')
    <h1>فراموشی رمز عبور</h1>
    <div class="subtitle">شماره موبایل خود را وارد کنید تا کد تایید برایتان پیامک شود</div>
@else
    <h1>تغییر رمز عبور</h1>
    <div class="subtitle">کد ارسال‌شده به <b>{{ $mobile }}</b> را همراه رمز جدید وارد کنید</div>
@endif
...
<input type="text" name="mobile" value="{{ old('mobile') }}" required inputmode="numeric" placeholder="09xxxxxxxxx" autofocus>
</div>
<button class="btn" type="submit">ارسال کد تایید</button>
</form>
@else
...
<div class="field">
<input type="text" name="code" class="code-input" required inputmode="numeric" maxlength="6" placeholder="------" autofocus>
</div>
...
<input type="password" id="pw-new1" name="password" required placeholder="حداقل ۶ کاراکتر">
<button class="eye" type="button" onclick="togglePassword('pw-new1', this)" title="نمایش رمز"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/></svg></button>
</div>
<label>تکرار رمز عبور جدید</label>
<div class="field">
<img class="ic" src="/icons/icon-lock.png" alt="">
<input type="password" id="pw-new2" name="password_confirmation" required placeholder="تکرار رمز عبور">
<button class="eye" type="button" onclick="togglePassword('pw-new2', this)" title="نمایش رمز"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/></svg></button>
</div>
@endif

<button class="btn" type="submit">{{ $mode === 'reset' ? 'تغییر رمز عبور' : 'تایید و ورود' }}</button>
</form>

<form class="resend-form" method="POST" action="{{ $resendAction }}">
@csrf
<button class="resend-btn" type="submit">ارسال مجدد کد</button>
</form>
@endif

<div class="foot">
@if ($mode === 'forgot')
    <a href="/panel/login">بازگشت به ورود</a>
@elseif ($mode === 'verify')
    <a href="/panel/register">بازگشت</a>
@else
    <a href="/panel/login">بازگشت به ورود</a>
@endif
</div>
