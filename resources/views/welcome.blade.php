...
<!-- SCB_CONTRAST_V2_START -->
<style>
/* اسکرول نرم برای کل صفحه */
html { scroll-behavior: smooth !important; }
@media (prefers-reduced-motion: reduce) { html { scroll-behavior: auto !important; } }

/* ===== ۱) کارت پلن‌ها/کارت‌ها — تاریک: پس‌زمینه تیره، متن روشن ===== */
html[data-theme="dark"] .plan,
html[data-theme="dark"] .card {
    background: rgba(30,41,59,.62) !important;
    border: 1px solid rgba(255,255,255,.18) !important;
    box-shadow: 0 18px 45px rgba(2,6,23,.5) !important;
}
html[data-theme="dark"] .plan h3,
html[data-theme="dark"] .card h3,
html[data-theme="dark"] .card h2 { color: #fff !important; }
html[data-theme="dark"] .plan li { color: #e2e8f0 !important; }
html[data-theme="dark"] .plan li::before { color: #4ade80 !important; }
html[data-theme="dark"] .plan p,
html[data-theme="dark"] .card p { color: #cbd5e1 !important; }

/* ===== حالت روشن: کارت سفید، متن تیره ===== */
...
html[data-theme="light"] .card h2 { color: #1e3a8a !important; }
html[data-theme="light"] .plan li { color: #334155 !important; }
html[data-theme="light"] .plan li::before { color: #16a34a !important; }
html[data-theme="light"] .plan p,
html[data-theme="light"] .card p { color: #64748b !important; }

/* ===== ۲) کنتراست همه دکمه‌ها ===== */
.btn, .btn-primary, .btn-blue, .btn-green, .btn-red, .btn-purple, .btn-cyan,
.btn-copy, .btn-search, .btn-dark, .btn-create, .btn-orange, .btn-gray,
.nav-btn, .logout, .btn-logout, #theme-toggle {
    color: #fff !important;
    text-shadow: none !important;
}
.btn-gray { background: #6b7280 !important; }
.btn-orange { background: linear-gradient(135deg, #f59e0b, #d97706) !important; }

/* دکمه روشن/تاریک در نوار بالا */
...
html[data-theme="light"] .nav #theme-toggle {
    ...
    box-shadow: 0 1px 4px rgba(15,23,42,.15) !important;
}
...
/* دکمه‌های هدر ادمین (تغییر رمز / خروج / روشن) */
header .logout, header #theme-toggle {
    background: rgba(255,255,255,.24) !important;
    color: #fff !important;
    border: 1px solid rgba(255,255,255,.55) !important;
}

/* دکمه گوشه بالا-چپ صفحات ورود/ثبت‌نام/تغییر رمز */
html[data-theme="dark"] .thm-fixed {
    background: rgba(2,6,23,.85) !important;
    color: #fff !important;
    border: 1px solid rgba(255,255,255,.45) !important;
}
html[data-theme="light"] .thm-fixed {
    background: #fff !important;
    color: #0f172a !important;
    border: 1px solid #94a3b8 !important;
...
/* دکمه‌های عملیات داخل جدول‌ها و فرم‌ها */
.actions .btn, .inline-form .btn, .export-buttons .btn, .upload-installer-form .btn { color: #fff !important; }
</style>
<!-- SCB_CONTRAST_V2_END -->

<!-- SCB_SCROLL_ANIM_START -->
<style>
html.scb-anim .reveal{
    opacity:0;
    transform:translateY(26px);
    transition:opacity .65s cubic-bezier(.22,.61,.36,1), transform .65s cubic-bezier(.22,.61,.36,1);
    will-change:opacity,transform;
}
html.scb-anim .reveal.scb-in{opacity:1;transform:translateY(0);}
html.scb-anim .reveal.scb-in.plan.featured{transform:translateY(-6px);}
html.scb-anim .grid-3 .reveal:nth-child(2), html.scb-anim .plans .reveal:nth-child(2), html.scb-anim .stats .reveal:nth-child(2){transition-delay:.08s;}
html.scb-anim .grid-3 .reveal:nth-child(3), html.scb-anim .plans .reveal:nth-child(3), html.scb-anim .stats .reveal:nth-child(3){transition-delay:.16s;}
html.scb-anim .stats .reveal:nth-child(4){transition-delay:.24s;}
@media (prefers-reduced-motion: reduce){
    html.scb-anim .reveal{opacity:1 !important;transform:none !important;transition:none !important;}
}
</style>
<!-- SCB_SCROLL_ANIM_END -->
...
<!-- SCB_ICONS_V1_START -->
<style>
.brand-logo{width:36px;height:36px;border-radius:10px;object-fit:cover;box-shadow:0 4px 12px rgba(2,6,23,.3);}
.admin-logo{width:40px;height:40px;border-radius:10px;vertical-align:middle;margin-left:10px;}
.ic{border-radius:6px;}
.eye-img{width:20px;height:20px;display:block;pointer-events:none;}
.ico-img{width:26px;height:26px;object-fit:cover;border-radius:8px;}
...
<!-- SCB_SCROLL_ANIM_JS_START -->
<script>
(function(){
    var doc=document.documentElement;
    doc.classList.add('scb-anim');
    var sel='.card, .plan, .stat, .cta, .section-title, .hero';
    if(!('IntersectionObserver' in window)){ doc.classList.remove('scb-anim'); return; }
    var els=document.querySelectorAll(sel);
    if(!els.length){ doc.classList.remove('scb-anim'); return; }
    var io=new IntersectionObserver(function(entries){
        entries.forEach(function(en){
            if(en.isIntersecting){ en.target.classList.add('scb-in'); io.unobserve(en.target); }
        });
    },{threshold:0.06, rootMargin:'0px 0px -30px 0px'});
    els.forEach(function(el){ el.classList.add('reveal'); io.observe(el); });
    setTimeout(function(){
        document.querySelectorAll('.reveal:not(.scb-in)').forEach(function(el){ el.classList.add('scb-in'); });
    },4000);
})();
</script>
<!-- SCB_SCROLL_ANIM_JS_END -->
